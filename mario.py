    ...
    def move(self, moving_left, moving_right):
        ...
        if self.alive == True:
            ...
            for question_box in question_box_group:
                if question_box.rect.colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                    dx = 0
                if question_box.rect.colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                    if abs((self.rect.top + dy) - question_box.rect.bottom) < collision_thresh:
                        self.vel_y = 0
                        dy = question_box.rect.bottom - self.rect.top
                        question_box.image = pygame.image.load('img/question_block2.png')
                        question_box.image = pygame.transform.scale(question_box.image, (TILE_SIZE, TILE_SIZE))

                    elif abs((self.rect.bottom + dy) - question_box.rect.top) < collision_thresh:
...
class Goomba(pygame.sprite.Sprite):
    ...
    #  for x in range(0, 3):
    #      img = pygame.image.load(f'img/goomba{x}.png')
    #      img = pygame.transform.scale(img, (TILE_SIZE, TILE_SIZE))
...
class QuestionBox(pygame.sprite.Sprite):
    ...
    def update(self):
        # Player.move.dx, Player.move.dy = Player.move()
        self.rect.x += screen_scroll
        
        
        # if pygame.sprite.spritecollide(self, player, False):
        #     self.image = pygame.image.load('img/question_block2.png').convert_alpha()
        #     self.image = pygame.transform.scale(self.image, (TILE_SIZE, TILE_SIZE))
...
class Exit(pygame.sprite.Sprite):
    ...
    # def show_dummy_coin(self):
    #     score_coin = Coin(TILE_SIZE // 2, TILE_SIZE // 2)
    #     coin_group.add(score_coin)
    #     score_coin.rect.x += screen_scroll
...
fire_plant_group = pygame.sprite.Group()
...
while run:
    ...
    if start_game == False:
        ...
        #check if player has collided with top of the goomba(Jumped on the enemy)
        # for goomba in goomba_group:
        #     if pygame.sprite.spritecollide(player, goomba, False):
        #         goomba.kill()
